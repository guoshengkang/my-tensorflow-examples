...
class CNN_LSTM(object):
    ...
    def __init__(self, regularizer_weight=0.001):
        ...
        self.W = []
        self.fnn_size=[8, 4, 1]  # 全连接每层的size
        self.LSTM_HIDDEN_SIZE=50 # LSTM每层的size
        self.LSTM_NUM_LAYERS=3   # LSTM隐层的数量

    def defineCNN(self,inputLayer=None):
        """
        定义卷积神经网络的结构
        """
        inputSize = inputLayer.shape[1].value
        inputLayer = tf.reshape(inputLayer, [-1, inputSize, 1])
        # (1) 定义卷积层1和池化层1，其中卷积层1里有4个feature map
        convPool1 = self.defineConvPool(inputLayer, channels=8,conv_size=3,conv_strides=1,
        pool_size=2,pool_strides=2)
        print('conv_pool_1 output size:',convPool1.get_shape().as_list())
        # (2) 定义卷积层2和池化层2，其中卷积层2里有8个feature map
        convPool2 = self.defineConvPool(convPool1, channels=12,conv_size=3,conv_strides=1,
        pool_size=2,pool_strides=2)
        ...
        self.CNN_out = tf.reshape(convPool2, [-1, pool_shape[1]*pool_shape[2]])
        print('transformed output size of CNN:',self.CNN_out.get_shape().as_list())

    def defineConvPool(self, inputLayer=None, channels=8,conv_size=4,conv_strides=1,
        pool_size=2,pool_strides=2):
        """
        定义卷积层和池化层
        inputLayer:输入tensor
    ...
    # LSTM结构单元
    def LstmCell(self):
        lstm_cell = tf.contrib.rnn.BasicLSTMCell(self.LSTM_HIDDEN_SIZE)
        return lstm_cell

    def defineLSTM(self,inputLayer=None):
        # 使用多层LSTM，不能用lstm_cell*NUM_LAYERS的方法，会导致LSTM的tensor名字都一样
        cell = tf.contrib.rnn.MultiRNNCell([self.LstmCell() for _ in range(self.LSTM_NUM_LAYERS)])
        # 将多层LSTM结构连接成RNN网络并计算前向传播结果
        inputLayer_shape=inputLayer.get_shape().as_list()
        inputLayer = tf.reshape(inputLayer, [-1, 1, inputLayer_shape[1]])
        output, _ = tf.nn.dynamic_rnn(cell, inputLayer, dtype=tf.float32)
        self.LSTM_out = tf.reshape(output, [-1, self.LSTM_HIDDEN_SIZE])
        print('LSTM_1-%d output size:'%self.LSTM_NUM_LAYERS,self.LSTM_out.get_shape().as_list())

    def defineFullConnected(self, inputLayer=None, size=[]):
        """
        定义全连接层的结构
        """
        ...
        for k,currentSize in enumerate(size[:-1]):
            ...
            layer += 1
            # 定义这一层神经元的输出
            neuralOut = tf.nn.tanh(tf.matmul(prevOut, weights) + biases)
        ...
        self.FNN_out = tf.reshape(fnn_out, [-1])
        print('transformed fnn final output size:',self.FNN_out.get_shape().as_list())

    def defineCNN1d_LSTM(self,inputLayer=None):
    	'''
    	定义CNN+LSTM+FNN
    	'''
    	self.defineCNN(inputLayer)
    	self.defineLSTM(inputLayer=self.CNN_out)
    	self.defineFullConnected(inputLayer=self.LSTM_out, size=self.fnn_size)
    ...
    def defineLoss(self):
        ...
        loss = tf.reduce_mean(tf.square(self.label - self.FNN_out))
        # L2惩罚项
    ...
    def fit(self, X=None, Y=None, startLearningRate=0.1, miniBatchFraction=0.1, epoch=3, keepProb=0.7):
        ...
        self.defineCNN1d_LSTM(inputLayer=self.input)
        self.defineLoss()
    ...
    def predict(self, X):
        ...
        """
        sess = self.sess
        prob = sess.run(self.FNN_out, feed_dict={self.input: X, self.keepProb: 1.0})
        if len(prob.shape)==2:
