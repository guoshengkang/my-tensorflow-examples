...
#定义神经网络的参数
#第一个隐层
w1 = tf.Variable(tf.random_normal([x_dim,unit_num],stddev=1.0/np.sqrt(float(x_dim)),seed=1),name='w1') #6235行,10列
b1 = tf.Variable(tf.zeros([1,unit_num]),name='b1') # 1,10
z1 = tf.matmul(x, w1)+b1 # 
a1 = tf.nn.tanh(z1) #使用tanh函数作为激活函数
#第二个隐层
w2 = tf.Variable(tf.random_normal([unit_num,unit_num],stddev=1.0/np.sqrt(float(unit_num)),seed=1),name='w2') #10行10列
b2 = tf.Variable(tf.zeros([1,unit_num]),name='b2') # 1,10
z2 = tf.matmul(a1, w2)+b2 # 
a2 = tf.nn.tanh(z2) #使用tanh函数作为激活函数
#第三个隐层
w3 = tf.Variable(tf.random_normal([unit_num,unit_num],stddev=1.0/np.sqrt(float(unit_num)),seed=1),name='w3') #10行10列
b3 = tf.Variable(tf.zeros([1,unit_num]),name='b3') # 1,10
z3 = tf.matmul(a2, w3)+b3 # 
a3 = tf.nn.tanh(z3) #使用tanh函数作为激活函数
#第四个隐层
w4 = tf.Variable(tf.random_normal([unit_num,unit_num],stddev=1.0/np.sqrt(float(unit_num)),seed=1),name='w4') #10行10列
b4 = tf.Variable(tf.zeros([1,unit_num]),name='b4') # 1,10
z4 = tf.matmul(a3, w4)+b4 # 
a4 = tf.nn.tanh(z4) #使用tanh函数作为激活函数
#定义神经网络输出层
w5 = tf.Variable(tf.random_normal([unit_num,1],stddev=1.0/np.sqrt(float(unit_num)),seed=1),name='w5')
...
tf.add_to_collection('losses',tf.contrib.layers.l2_regularizer(regularizer_weight)(w5))
# 将均方误差损失函数加入损失集合
